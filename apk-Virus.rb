...
num=gets.chomp.to_i
 if num ==1
   system("unzip apk.zip")
   puts "File decompression finished"
   system("cp -r apk/sdcard")
   puts "A file has been moved to sdcard"
   puts "[Note] No application has been modified or cracked. We are not interested in modifying the applications without understanding the way of making and not having enough time to make something like this. I decided to use other people's applications. This does not mean that I steal the application or anything. I just contributed to spreading the application"
